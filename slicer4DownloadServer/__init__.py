...
def redirectToLocalBitstream():
    ...
    if error_code in (400, 404):
        return (flask.render_template('{0}.html'.format(error_code),
...
def recordMatching():
    """High level function for getting all records matching specific criteria
    including OS.
    """
    ...
    if os not in SupportedOSChoices:
        return (None,
                'unknown os "{0}": '
                'should be one of {1}'.format(os, SupportedOSChoices),
                400)
    ...
    if len(modeDict.keys()) == 0:
        ...
        return (None,
                "invalid or ambiguous mode: "
                "should be one of {0}".format(ModeChoices),
                400)
    ...
    if stability not in StabilityChoices:
        return (None,
                "bad stability {0}: "
                "should be one of {1}".format(stability, StabilityChoices),
                400)
...
def recordsMatchingAllOSAndStability():
    ...
    results = {}
    for supported_os in SupportedOSChoices:
        osResult = {}
        for stability in ('release', 'nightly'):
            r = getBestMatching(revisionRecords, supported_os, stability,
                                modeName, value, offset)
            osResult[stability] = cleanupMidasRecord(r)
        results[supported_os] = osResult

    return results, None, 200


# query matching functions
def matchOS(selected_os):
    return lambda r: r['os'] == selected_os
...
def allPass(predlist):
    """returns a function that evaluates each predicate in a list given
    an argument, and returns True if all pass, otherwise False."""
    def pred(x):
        for p in predlist:
            if not p(x):
                return False
...
def getBestMatching(revisionRecords, os, stability, mode, modeArg, offset):
    ...
    if matchingRecordIndex == -1:
        ...
        if offset < 0:
            ...
            # look forward in time for the latest build of a particular
            # rev, so flip list
